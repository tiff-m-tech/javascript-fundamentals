...
/*
    Use a 'for...in' loop to iterate through the 'stationPingsMs' object.
...
for (let stationKey in stationPingsMs) {
	let latencyValue = stationPingsMs[stationKey];
	console.log(`Connection to ${stationKey} latency is: ${latencyValue}ms.`);
}
...
/*
    Use 'Object.keys()' to extract an array of all keys from 'pdcAmmoLocker'.
    Iterate over that keys array using a 'for...of' loop.
    Inside the loop, check if the value of that key equals 0. If it does, 
    log an emergency update using a template literal: "CRITICAL DEPLETION: [key] ammo is completely empty!"
*/
let lockerKeys = Object.keys(pdcAmmoLocker);

for (let currentKey of lockerKeys) {
	if (pdcAmmoLocker[currentKey] === 0) {
		console.log(`CRITICAL DEPLETION: ${currentKey} ammo is completely empty!`);
	}
}
