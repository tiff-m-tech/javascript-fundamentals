/**** LOOPING OVER ARRAYS OF OBJECTS: PRACTICE SOLUTION ****/
...
/*
    Use a 'for...of' loop to scan through the 'atmosphericScrubbers' array.
...
for (let scrubber of atmosphericScrubbers) {
	if (scrubber.filterLifePercent < 40) {
		console.log(
			`MAINTENANCE REQ: ${scrubber.zone} filter core is critically low at ${scrubber.filterLifePercent}%!`
		);
	}
}
...
/*
    Write a standard 'for' loop using index tracking counters ('i') to scan the list.
...
for (let i = 0; i < targetDrones.length; i++) {
	totalThreatSum += targetDrones[i].threatScore;
}

let compositeAverage = totalThreatSum / targetDrones.length;
console.log(`Target grid composite average threat rating: ${compositeAverage}`);
